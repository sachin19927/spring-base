...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleMalformedRequest(HttpMessageNotReadableException ex, HttpServletRequest request){
        ...
        return ResponseEntity.badRequest().body(ApiProblemDetailsFactory.create(
    ...
    public ResponseEntity<ProblemDetail> handleRequestValidations(MethodArgumentNotValidException ex, HttpServletRequest request){
        var fieldErrors = ex.getBindingResult().getFieldErrors().stream()
                .map(error-> new ApiFieldError(error.getField(), Objects.requireNonNull(error.getDefaultMessage()))).toList();
        return ResponseEntity.badRequest().body(ApiProblemDetailsFactory.create(
                HttpStatus.BAD_REQUEST,
    ...
    public ResponseEntity<ProblemDetail> handleApiException(ApiException ex, HttpServletRequest request){
       ApiProblemType problemType = ex instanceof ResourceNotFoundException ? ApiProblemType.RESOURCE_NOT_FOUND : ApiProblemType.BUSINESS_VALIDATION;
    ...
    public ResponseEntity<ProblemDetail> handleDataIntegrityViolations(DataIntegrityViolationException ex, HttpServletRequest request){
        ...
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ApiProblemDetailsFactory.create(
                HttpStatus.CONFLICT,
    ...
    public ResponseEntity<ProblemDetail> handleUnwantedException(Exception ex, HttpServletRequest request){
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiProblemDetailsFactory.create(
